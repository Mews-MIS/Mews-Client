...
const NewsPage = () => {
  ...
  const [articles, setArticles] = useState<NewsListItemProps[] | null>([]);
  ...
  return (
    <s.Wrapper>
      ...
      <s.NewsbottomContainer>
        <s.NewsListBox>
          {articles
            ? articles.map(({ contentInfo }: NewsListItemProps) => {
                return <NewsListItem key={contentInfo.id} index={contentInfo.index} />;
              })
            : "등록된 게시물이 없습니다"}
        </s.NewsListBox>
